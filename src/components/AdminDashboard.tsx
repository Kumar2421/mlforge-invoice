...
export default function AdminDashboard() {
  ...
  return (
    <div className="flex h-screen min-w-[1024px] overflow-hidden bg-[#F6F8F7] text-[#17221F]">
      <aside className="flex w-60 shrink-0 flex-col border-r border-[#E7ECE9] bg-white">
        <div className="flex h-16 items-center gap-2.5 px-5">
          <MLForgeMark className="h-8 w-8 shrink-0" />
          <div>
            <p className="text-[13px] font-black tracking-tight">Payment Reminders</p>
            <p className="text-[9px] font-bold uppercase tracking-widest text-[#0F5A68]">Platform admin</p>
          </div>
        </div>

        {/* Go to App Workspace Button */}
        <div className="px-3 py-2 border-b border-[#E7ECE9]">
          <button
            onClick={() => { window.location.href = "/dashboard"; }}
            className="flex w-full items-center gap-2.5 rounded-xl bg-[#0F5A68] hover:bg-[#0C4D59] px-3 py-2 text-left text-[11px] font-bold text-white transition-colors"
          >
        ...
        <div className="px-3 py-3">
          <p className="px-3 text-[9px] font-bold uppercase tracking-widest text-gray-400">Operate</p>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto px-3">
          {nav.map(({ name, icon: Icon }) => (
            <button
              ...
              className={`mb-1 flex w-full items-center gap-2.5 rounded-xl px-3 py-2.5 text-left text-[11px] font-bold transition-colors ${
                activePage === name
                  ? "bg-[#EAF3F0] text-[#0F5A68]"
            ...
            >
              ...
              {name}
              {name === "Reminder Ops" && (
                <span className="ml-auto rounded-full bg-rose-100 px-1.5 py-0.5 text-[9px] text-rose-600">3</span>
              )}
            </button>
          ))}
        </nav>

        <div className="m-3 rounded-2xl bg-[#143D4A] p-3 text-white">
          <p className="text-[9px] font-bold uppercase tracking-widest text-white/50">Internal only</p>
          <p className="mt-1 text-[10px] font-semibold leading-relaxed text-white/80">
            Every production action will be permissioned and auditable.
          </p>
        </div>
      </aside>

      <div className="flex min-w-0 flex-1 flex-col">
        <header className="flex h-16 shrink-0 items-center justify-between border-b border-[#E7ECE9] bg-white px-6">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-widest text-[#0F5A68]">Platform control</p>
            <h1 className="mt-0.5 text-[17px] font-black tracking-tight">{activePage}</h1>
          </div>
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              ...
              >
                ...
                <span className="absolute right-1.5 top-1.5 h-1.5 rounded-full bg-rose-500" />
              </button>
              {showAlerts && (
                <div className="absolute right-0 mt-2 w-64 bg-white border border-gray-200 rounded-lg shadow-lg z-50 p-3 text-left">
                  <h4 className="text-[11px] font-bold text-gray-900 mb-2">Platform Alerts</h4>
                  <div className="text-[10px] text-gray-500">No new critical alerts.</div>
                </div>
              )}
            </div>
            <div className="ml-1 flex h-8 w-8 items-center justify-center rounded-full bg-[#DCECE7] text-[11px] font-black text-[#0F5A68]">
              OP
            </div>
          </div>
        </header>
            ...
            <div className="mb-5 flex items-end justify-between">
              <div>
                <h2 className="text-[20px] font-black tracking-tight">
                  {activePage === "Overview" ? "Good morning, Operator" : activePage}
                </h2>
                <p className="mt-1 text-[11px] font-medium text-gray-400">Phase 1 UI prototype — local mock data only.</p>
              </div>
              <button className="flex items-center gap-2 rounded-full bg-[#0F5A68] px-3.5 py-2 text-[10px] font-bold text-white hover:bg-[#0C4D59]">
